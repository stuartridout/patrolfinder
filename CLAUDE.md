...
Last: 29 Aug 2026 - Third phone review. The 120 postmark is rebuilt: place over the top, date arced under the foot on the same band, "120 YEARS" centred and the number larger. Fixing it turned up that the canvas copy had been drawing "BROWNSEA ISLAND" upside down all along, because a bottom arc and a top arc sweep opposite ways and `arcText` only knew one of them. On the Instagram cards nothing is laid out against a fixed column any more: `roomFor()` asks how far a line may run at that line's own height, which took the eyebrow's clearance from the roundel from 30-40px to 74-90px on a 1080 card. The story's type now grows a little as well as spreading out, which was half of an earlier ask that only got the spacing.
