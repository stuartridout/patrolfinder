...
Last: 29 Aug 2026 - Copy fixes and pull-to-refresh on the wall. Q1's Wolves option is now "Assigning jobs to people. Every job gets a buddy" and the Ravens drying-line option finishes its sentence. The wall pulls down to refresh, with a Refresh button beside a timestamp for laptops; a failed refresh keeps the cards that are already up. The pull had to move an inner wrapper rather than `.screen`, whose fadeup animation has fill mode both and silently overrode the inline transform. Still open: what a kudu horn is - a silhouette of one does not read at any size, so it needs either an image from Stuart or a few words in the prompt.
