...
- The icon is the four-patrol-colour roundel, and it stays. A Baden-Powell silhouette was tried and rejected: at icon size a head in a wide-brimmed hat reads as a plague doctor's mask, which is not a thing you can tune your way out of. `icons/favicon.svg` is the only place it is drawn and the eight PNGs are generated from it; never hand-edit them.
...
Last: 29 Aug 2026 - Fourth phone review. On the Instagram cards the link is now a pill in the bottom right corner rather than the last line of the stack, which lets the whole text block drop far enough to clear the torn edge - it wobbles by W*0.026 and could reach the eyebrow. The two lines that end up beside the pill are measured against it, the same way every line is already measured against the roundel. The Baden-Powell icon is reverted to the original four-patrol roundel: at icon size a head in a wide-brimmed hat read as a plague doctor's mask.
