...
Last: 29 Aug 2026 - Second phone review, five changes, live. The zoom slider is gone: the picture is pinched and dragged directly, with the wheel for laptops and arrows/+- on the focusable canvas so the keyboard keeps both. Wall cards lost the patrol pill that was landing on the card's own name, and Report became a transparent 44px button with the pill on an inner span, because painting the button and clipping to its content box made a pill exactly as wide as the word. The five patrol filters now share one row instead of wrapping Ravens onto its own line. The ribbon is off the card studio, where it ran into the white banner and read as a glitch.
